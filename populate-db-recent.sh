...
for table in instrument_upload model_upload regular_file model_file search_file; do
  oc exec deploy/postgres -i -- psql -U dataportal_ro dataportal > "$tmp_dir/$table.sql" << SQL
COPY (SELECT * FROM $table
      WHERE "measurementDate" >= '$start_date') TO STDOUT;
SQL
done
oc exec deploy/postgres -i -- psql -U dataportal_ro dataportal > "$tmp_dir/visualization.sql" << SQL
COPY (SELECT visualization.*
      ...
      WHERE "measurementDate" >= '$start_date') TO STDOUT;
SQL
oc exec deploy/postgres -i -- psql -U dataportal_ro dataportal > "$tmp_dir/model_visualization.sql" << SQL
...
for table in regular_file model_file; do
  ...
  oc exec deploy/postgres -i -- psql -U dataportal_ro dataportal > "$tmp_dir/$tmp_table.sql" << SQL
...
oc exec deploy/postgres -i -- psql -U dataportal_ro dataportal > "$tmp_dir/file_quality.sql" << SQL
COPY (SELECT file_quality.*
      ...
      WHERE "measurementDate" >= '$start_date') TO STDOUT;
SQL
oc exec deploy/postgres -i -- psql -U dataportal_ro dataportal > "$tmp_dir/quality_report.sql" << SQL
